/**
 * MarkdownGenerator - AI-Ready Output Generator for MarkuprX
 *
...
export function adaptFeedbackSession(
  ...
  return {
    ...
    metadata: {
      ...
      sourceName: `${PUBLIC_BRAND_NAME} Session`,
      sourceType: 'screen',
    },
  };
}
...
class MarkdownGeneratorImpl implements IMarkdownGenerator {
  ...
  generateFullDocument(session: Session, options: GenerateOptions): MarkdownDocument {
    ...
    const markedIssues = session.metadata?.markedIssues ?? [];
    const totalItemCount = items.length + markedIssues.length;
    ...
    if (items.length === 0 && markedIssues.length === 0) {
      const content = `# ${projectName} Feedback Report
> Generated by ${PUBLIC_BRAND_NAME} on ${timestamp}
> Duration: ${duration}
...
`;
    ...
    if (markedIssues.length > 0) typeCounts['Marked Issue'] = markedIssues.length;
    const severityCounts = this.countSeverities(items);
    if (markedIssues.length > 0) {
      severityCounts.Medium = (severityCounts.Medium || 0) + markedIssues.length;
    }
    const screenshotCount = this.countScreenshots(items)
      + markedIssues.filter((issue) => Boolean(issue.screenshotPath)).length;
...
> Generated by ${PUBLIC_BRAND_NAME} on ${timestamp}
> Duration: ${duration} | Items: ${totalItemCount} | Screenshots: ${screenshotCount}
...
- **Marked issues:** ${markedIssues.length}
...
- ${totalItemCount} total feedback items were captured.
...
*Generated by [${PUBLIC_BRAND_NAME}](${PUBLIC_WEBSITE_URL})*
`;
    content = insertMarkedIssuesSection(content, markedIssues, screenshotDir);
  ...
  generateFromPostProcess(result: PostProcessResult, sessionDir: string): string {
    ...
    if (transcriptSegments.length === 0) {
      ...
      if (ordinaryFrames.length > 0) {
        md += `## Captured Moments\n\n`;
        for (const frame of ordinaryFrames.slice().sort((a, b) => a.timestamp - b.timestamp)) {
    ...
    const segmentFrameMap = this.mapFramesToSegments(transcriptSegments, ordinaryFrames);
