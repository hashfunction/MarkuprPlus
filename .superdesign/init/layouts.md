...
The renderer is a single Electron page rather than a URL-routed web app. `AppWrapper` is the provider shell and `App` is the shared visual shell for the home view plus secondary overlays.

## AppWrapper

- Source: `src/renderer/AppWrapper.tsx`
- Purpose: Composes the error boundary and the recording, processing, and UI providers.
...
- Purpose: The shared portrait app shell. It owns the main header, state-specific content, and mounts all secondary views as overlays.
