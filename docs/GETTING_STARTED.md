...
### Step 4: Mark Separate Issues Without Losing Mouse Control

Keep talking as you review. Hold **Command (`⌘`) on macOS** or **Control (`Ctrl`) on Windows**, then drag over the recorded area. Normal mouse interaction stays enabled whenever the modifier is not held. The protected toolbar offers:

- Pen, circle, and translucent highlighter tools
- Red, yellow, green, and blue
- Undo and Clear

Release Command or Control when the mark is complete. The drawing remains visible for review. Your next ordinary click:

- reaches the underlying application normally;
...
2. Review your feedback items and extracted frames; every marked issue has its own annotated screenshot and matching comment
