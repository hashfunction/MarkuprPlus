...
# I Built an MCP Server That Lets Claude Code See Your Screen
...
**Target platforms:** Hashnode (primary, canonical URL), Dev.to (cross-post), personal blog if applicable.

**Tags:** `mcp`, `ai`, `developer-tools`, `open-source`, `claude`, `typescript`, `webdev`

**Assets needed before publishing:**
- [ ] Demo GIF: full loop (hotkey -> narrate -> markdown output)
- [ ] MCP demo GIF: Claude Code calling `capture_screenshot` and fixing a bug
- [ ] Place GIFs inline where `[Demo GIF placeholder]` comments would go (after "The Solution" section and in "The MCP Angle" section)

**Word count:** ~1,500 words (excluding code blocks and metadata)
