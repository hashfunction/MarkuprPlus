...
describe('formatIssueBody', () => {
  ...
  it('includes screenshot count when screenshots exist', () => {
    ...
    expect(body).toContain('see the MarkuprPlus report for images');
    expect(body).not.toContain('MarkuprX');
  });
  ...
  it('includes canonical MarkuprPlus attribution', () => {
    const item = makeFeedbackItem();
    const body = formatIssueBody(item);
    expect(body).toContain('[MarkuprPlus](https://markuprplus.com)');
    expect(body).not.toContain('MarkuprX');
    expect(body).not.toContain('markuprx.com');
  });
});
...
describe('getLabelsForItem', () => {
  it('always includes the MarkuprPlus label', () => {
    const item = makeFeedbackItem();
    const labels = getLabelsForItem(item);
    expect(labels).toContain('MarkuprPlus');
  });
  ...
  it('returns exactly 3 labels (MarkuprPlus + category + severity)', () => {
    const item = makeFeedbackItem({ category: 'Bug', severity: 'High' });
    const labels = getLabelsForItem(item);
    expect(labels).toHaveLength(3);
    expect(labels).toEqual(['MarkuprPlus', 'bug', 'priority: high']);
  });
});
...
describe('collectRequiredLabels', () => {
  it('always includes the MarkuprPlus label', () => {
    ...
    expect(labels[0]).toMatchObject({
      name: 'MarkuprPlus',
      description: 'Created from MarkuprPlus feedback session',
    });
  });
  ...
  it('deduplicates labels across items', () => {
    ...
    // MarkuprPlus, bug, priority: high -- no duplicates
    expect(names).toEqual(['MarkuprPlus', 'bug', 'priority: high']);
  });
...
describe('GitHubAPIClient', () => {
  ...
  describe('createIssue', () => {
    it('sends correct request and returns issue data', async () => {
      ...
      expect(body.labels).toEqual(['bug', 'MarkuprPlus']);
    });
  ...
  describe('ensureLabel', () => {
    ...
    it('creates label if it does not exist', async () => {
      ...
      expect(createBody.name).toBe('MarkuprPlus');
...
describe('pushToGitHub', () => {
  ...
  it('dry-run returns issues without making API calls', async () => {
    ...
    expect(result.labelsCreated).toContain('MarkuprPlus');
  ...
  it('throws on empty report', async () => {
    ...
    await expect(
      ...
      'No feedback items found in the report. Is this a valid MarkuprPlus report?',
    );
  });
  ...
  it('creates issues in real mode with correct API calls', async () => {
    ...
    // ensureLabel calls (MarkuprPlus + bug + performance + enhancement + critical + high + low)
